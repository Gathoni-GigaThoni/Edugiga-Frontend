...
// Clickable document reference — attached by the BE to any report row that
...
const REPORT_DEFS = {
  ...
  // Rendered by _repRenderTaxSchedules (nested groups: tax_type →
  // {tax_vouchers, wht_certificates, total}). Pre-Wave-4 the columns
  // config here was aspirational and never matched the payload; the
  // report fell to _repRenderTable which couldn't find any list under
  // the {grouped_by_tax_type: {...}} shape and always rendered "No data".
  ...
  // Rendered by _repRenderDailyCashReturn (three parallel row lists
  // — receipts / vouchers / petty cash — plus opening/closing summary).
  // No `columns` config: the pre-2026-09-07 ['account','opening_balance',
  // 'inflows','outflows','closing_balance'] list didn't match the payload
  // at all and rendered as "No data" through _repRenderTable.
  ...
  // Rendered by _repRenderTendepayHistory (multi-ref per row: PV +
  // JE deep-links). Pre-Wave-5 the columns config referenced a
  // `tendepay_status` field that never came back from the BE and
  // omitted both matched_voucher_id and journal_entry_id, so the two
  // most useful cross-references (which PV and which JE settled this
  // tendepay transaction) were invisible on screen.
...
function _repRenderTable(def, data) {
  ...
  // Per-column doc_ref support: if def.docRefKeys lists a column key,
  // that cell renders as a clickable link when the row carries a doc_ref
  ...
  if (def.groupBy && knownKeysPresent) {
    ...
    bodyRows = sortedRows.map(r => {
      ...
      return `<tr style="${toggle ? 'background:#f5fafa;' : ''}">${cols.map(([k]) => `<td>${renderCell(r, k)}</td>`).join('')}</tr>`;
    }).join('');
  } else {
    bodyRows = sortedRows.map(r => `<tr>${cols.map(([k]) => `<td>${renderCell(r, k)}</td>`).join('')}</tr>`).join('');
  }
...
function _repRenderLedgerLines(def, data) {
  ...
  const bodyRows = rows.map(r => {
    ...
    // jv_number is a clickable doc_ref when the BE ships one; falls back
    // to plain text on older payloads (no BE/FE deploy skew guard needed).
    return `<tr>${cols.map(([k]) => {
      if (k === 'jv_number') return `<td>${_repDocLink(r.doc_ref) || _repCell(view[k])}</td>`;
      return `<td>${_repCell(view[k])}</td>`;
    }).join('')}</tr>`;
  }).join('');
  const totalsRow = cols.map(([k], i) => {
    if (i === 0) return '<td><strong>TOTAL</strong></td>';
...
// ── Daily Cash Return ──────────────────────────────────────────────────────
// Three parallel sections + opening/closing summary. Pre-2026-09-07 this
// report fell through to the generic table renderer, which couldn't find
// any of {cash_receipts, cash_payments_vouchers, petty_cash_disbursements}
// and rendered "No data" regardless of what the BE returned. Receipt +
// voucher numbers are clickable when the BE ships a doc_ref (Wave 3);
// petty cash rows have no owning detail view and stay as plain PC ids.
function _repRenderDailyCashReturn(def, data) {
  const out = document.getElementById('rep-output');
  if (!data || typeof data !== 'object') {
    out.innerHTML = '<p class="fin-empty">No data for the selected date.</p>';
    return;
  }
  const receipts = Array.isArray(data.cash_receipts) ? data.cash_receipts : [];
  const vouchers = Array.isArray(data.cash_payments_vouchers) ? data.cash_payments_vouchers : [];
  const petty    = Array.isArray(data.petty_cash_disbursements) ? data.petty_cash_disbursements : [];

  const summaryBox = `
    <div class="fin-form-wrap" style="max-width:520px;margin-bottom:14px;">
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span>Report Date</span><span>${_pvDate(data.report_date)}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span>Opening Cash</span><span>${_pvMoney(data.opening_cash)}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span>Total Receipts</span><span>${_pvMoney(data.total_receipts)}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span>Total Payments</span><span>${_pvMoney(data.total_payments)}</span></div>
      <div style="display:flex;justify-content:space-between;padding:6px 0;font-weight:bold;border-top:2px solid #2c3e50;"><span>Closing Cash</span><span>${_pvMoney(data.closing_cash)}</span></div>
    </div>`;

  const section = (label, headers, rows, emptyMsg, totalCell) => {
    const body = rows.length
      ? rows.map(r => `<tr>${r.map(c => `<td>${c}</td>`).join('')}</tr>`).join('')
      : `<tr><td colspan="${headers.length}" class="fin-empty">${_finEsc(emptyMsg)}</td></tr>`;
    const tfoot = rows.length && totalCell != null
      ? `<tfoot><tr class="fin-tfoot-total">${
          headers.map((_, i) =>
            i === 0 ? '<td><strong>TOTAL</strong></td>'
            : i === headers.length - 1 ? `<td><strong>${_pvMoney(totalCell)}</strong></td>`
            : '<td></td>'
          ).join('')
        }</tr></tfoot>`
      : '';
    return `
      <div class="fin-section-label" style="margin-top:16px;">${_finEsc(label)}</div>
      <div class="fin-table-wrap"><table class="fin-table">
        <thead><tr>${headers.map(h => `<th>${_finEsc(h)}</th>`).join('')}</tr></thead>
        <tbody>${body}</tbody>
        ${tfoot}
      </table></div>`;
  };
  ...
  const receiptsRows = receipts.map(r => [
    _repDocLink(r.doc_ref) || _finEsc(r.receipt_number || '—'),
    _finEsc(typeof _rcvStudentName === 'function' ? _rcvStudentName(r.student_id) : `#${r.student_id}`),
    _finEsc(r.reference || '—'),
    _pvMoney(r.amount),
  ]);
  const vouchersRows = vouchers.map(v => [
    _repDocLink(v.doc_ref) || _finEsc(v.voucher_no || '—'),
    _finEsc(v.payee_name || '—'),
    _finEsc(v.description || '—'),
    _pvMoney(v.amount),
  ]);
  const pettyRows = petty.map(p => [
    _finEsc(`PC-${p.disbursement_id}`),
    _pvMoney(p.disbursed_amount),
  ]);

  out.innerHTML = summaryBox
    + section('Cash Receipts',
        ['RECEIPT NO.', 'STUDENT', 'REFERENCE', 'AMOUNT'],
        receiptsRows, 'No cash receipts on this date.', data.total_receipts)
    + section('Cash Payment Vouchers',
        ['VOUCHER NO.', 'PAYEE', 'DESCRIPTION', 'AMOUNT'],
        vouchersRows, 'No cash payment vouchers on this date.', data.total_payments)
    + section('Petty Cash Disbursements',
        ['DISBURSEMENT', 'AMOUNT'],
...
// total). Pre-Wave-4 this report fell through the generic table renderer
// which couldn't find any top-level list under the {grouped_by_tax_type}
// shape and always showed "No data" regardless of activity. Voucher
// numbers deep-link to the paying PV (doc_ref); WHT certificate numbers
// stay plain — there's no cert detail view.
function _repRenderTaxSchedules(def, data) {
  const out = document.getElementById('rep-output');
  if (!data || typeof data !== 'object') {
    out.innerHTML = '<p class="fin-empty">No data for the selected period.</p>';
    return;
  }
  const grouped = data.grouped_by_tax_type || {};
  const keys = Object.keys(grouped);
  if (!keys.length) {
    out.innerHTML = '<div class="fin-table-wrap"><table class="fin-table"><tbody><tr><td class="fin-empty">No tax vouchers or WHT certificates in the selected period.</td></tr></tbody></table></div>';
    return;
  }

  const groupHtml = keys.sort().map(taxType => {
    const g = grouped[taxType] || {};
    const tvs = Array.isArray(g.tax_vouchers) ? g.tax_vouchers : [];
    const certs = Array.isArray(g.wht_certificates) ? g.wht_certificates : [];

    const tvRows = tvs.length ? tvs.map(tv => `<tr>
      <td>${_repDocLink(tv.doc_ref) || _finEsc(tv.voucher_no || '—')}</td>
      <td>${_finEsc((tv.period_month || '') + '/' + tv.period_year)}</td>
      <td>${_pvDate(tv.remittance_due_date)}</td>
      <td>${_finEsc(tv.kra_payment_slip_ref || '—')}</td>
      <td>${_pvMoney(tv.amount)}</td>
    </tr>`).join('')
      : '<tr><td colspan="5" class="fin-empty">No tax vouchers in this group.</td></tr>';

    const certRows = certs.length ? certs.map(c => `<tr>
      <td>${_finEsc(c.certificate_number || '—')}</td>
      <td>${_finEsc(typeof _pvSupplierName === 'function' ? _pvSupplierName(c.supplier_id) : '#' + c.supplier_id)}</td>
      <td>${_pvDate(c.issue_date)}</td>
      <td>${_pvMoney(c.gross_amount)}</td>
      <td>${_pvMoney(c.wht_rate)}</td>
      <td>${_pvMoney(c.wht_amount)}</td>
    </tr>`).join('') : '';

    return `
      <div class="fin-section-label" style="margin-top:18px;">${_finEsc(taxType)} — Total ${_pvMoney(g.total)}</div>
      <div class="fin-table-wrap"><table class="fin-table">
        <thead><tr><th>REMITTANCE PV</th><th>PERIOD</th><th>DUE DATE</th><th>KRA SLIP</th><th>AMOUNT</th></tr></thead>
        <tbody>${tvRows}</tbody>
      </table></div>
      ${certs.length ? `
        <div style="font-size:0.82rem;color:#555;margin:10px 0 4px;">WHT Certificates issued in period</div>
        <div class="fin-table-wrap"><table class="fin-table">
          <thead><tr><th>CERTIFICATE NO.</th><th>SUPPLIER</th><th>ISSUE DATE</th><th>GROSS</th><th>RATE</th><th>WHT</th></tr></thead>
          <tbody>${certRows}</tbody>
        </table></div>` : ''}`;
  }).join('');

  out.innerHTML = groupHtml;
}

// ── Tendepay Transaction History ────────────────────────────────────────────
// Multi-ref per row: the same tendepay transaction can point at both a PV
// (via matched_voucher_ref) and a JE (via journal_entry_ref). Both refs
// are nullable — a suspense-posted tx that was never later matched has
// neither. Pre-Wave-5 the FE dropped both id columns entirely, so users
// investigating a Tendepay drift couldn't jump from the tx to the paying
// PV or the accrual JE without cross-referencing them by hand.
function _repRenderTendepayHistory(def, data) {
  const out = document.getElementById('rep-output');
  const rows = Array.isArray(data) ? data : (data && data.data) || [];
  if (!rows.length) {
    out.innerHTML = '<div class="fin-table-wrap"><table class="fin-table"><tbody><tr><td class="fin-empty">No posted Tendepay transactions in the selected period.</td></tr></tbody></table></div>';
    return;
  }
  const bodyRows = rows.map(r => `<tr>
    <td>${_pvDate(r.transaction_date)}</td>
    <td>${_finEsc(r.tendepay_reference || '—')}</td>
    <td>${_finEsc(r.wallet_name || '—')}</td>
    <td>${_finEsc(r.payee_name || '—')}</td>
    <td>${_repDocLink(r.matched_voucher_ref) || (r.matched_voucher_id ? '#' + r.matched_voucher_id : '—')}</td>
    <td>${_repDocLink(r.journal_entry_ref) || (r.journal_entry_id ? '#' + r.journal_entry_id : '—')}</td>
    <td>${_pvMoney(r.amount)}</td>
  </tr>`).join('');
  ...
  out.innerHTML = `
    <div class="fin-table-wrap"><table class="fin-table">
      <thead><tr>
        <th>DATE</th><th>TENDEPAY REF</th><th>WALLET</th><th>PAYEE</th>
        <th>PV</th><th>JE</th><th>AMOUNT</th>
      </tr></thead>
      <tbody>${bodyRows}</tbody>
      <tfoot><tr class="fin-tfoot-total">
...
function _repRenderAgedStudentDebtors(data) {
  ...
  const bucketCell = (rows) => {
    ...
    // Each invoice_number is clickable when the BE ships a doc_ref;
    // falls back to plain escaped text on older payloads.
    const refs = rows.map(r => _repDocLink(r.doc_ref) || _finEsc(r.invoice_number)).join(', ');
...
const _REP_FEES_PER_GL = {
  invoiced: {
    ...
    childCells: r => [
      // Wave 3: clickable when the BE ships a doc_ref (INV_STUDENT);
      // plain text on older payloads.
      _repDocLink(r.doc_ref) || _finEsc(r.invoice_number || '—'),
...
function _repRenderApReconciliation(def, data) {
  ...
  // Anomalies — data-integrity smells surfaced independently of the drift
  // math. A clean total can still mask sick rows (e.g. a PENDING invoice
  // that still carries an accrual JE), so this section must render even
  // when is_reconciled=True. Pre-Wave-4 the FE renderer didn't show
  // anomalies at all — the BE emitted them but the data was invisible.
  const anomalies = data.anomalies || [];
  const anomaliesSection = anomalies.length ? `
    <div class="fin-section-label" style="margin-top:20px;color:var(--coral-600);">Anomalies — data-integrity smells (independent of the drift math)</div>
    <div class="fin-table-wrap"><table class="fin-table">
      <thead><tr>
        <th>INVOICE</th><th>SUPPLIER</th><th>STATUS</th>
        <th>ACCRUAL JE</th><th>ANOMALY</th><th>MESSAGE</th>
      </tr></thead>
      <tbody>${anomalies.map(a => `<tr>
        <td>${_repDocLink(a.doc_ref) || _finEsc(a.invoice_number || ('#' + a.invoice_id))}</td>
        <td>${_finEsc(_pvSupplierName(a.supplier_id))}</td>
        <td>${_finEsc(a.status || '—')}</td>
        <td>${_repDocLink(a.accrual_je_ref) || (a.accrual_journal_entry_id ? '#' + a.accrual_journal_entry_id : '—')}</td>
        <td>${_finEsc(a.anomaly_type || '—')}</td>
        <td style="font-size:0.85rem;">${_finEsc(a.message || '')}</td>
      </tr>`).join('')}</tbody>
    </table></div>` : '';
  ...
  const driftRows = (data.drift_invoices || []).map(d => {
    const invLink = _repDocLink(d.doc_ref)
      || `<a href="#payables-supplier-invoices?open=${d.invoice_id}" target="_blank" rel="noopener">${_finEsc(d.invoice_number || ('#' + d.invoice_id))}</a>`;
    return `<tr>
      <td>${invLink}</td>
...
function _repRenderSupplierStatement(def, data) {
  ...
  // Explicit column list — replaces def.columns fallback so we can slot
  // in the ACCRUAL JE column (Wave 4) between REFERENCE and AMOUNT and
  // control cell rendering per key without hunting through the def.
  const cols = [
    ['invoice_date',    'DATE'],
    ['invoice_number',  'REFERENCE'],
    ['accrual_je_ref',  'ACCRUAL JE'],
    ['amount',          'AMOUNT'],
    ['running_balance', 'RUNNING BALANCE'],
  ];

  const renderCell = (r, k) => {
    if (k === 'invoice_number') return _repDocLink(r.doc_ref) || _repCell(r[k]);
    if (k === 'accrual_je_ref') return _repDocLink(r.accrual_je_ref) || '—';
    return _repCell(r[k]);
  };

  const bodyRows = rows.map(r => {
    const cells = cols.map(([k]) => `<td>${renderCell(r, k)}</td>`).join('');
