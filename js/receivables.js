...
async function openFeeScheduleModal(editId) {
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:8px;padding:28px;max-width:520px;width:92%;max-height:90vh;overflow-y:auto;">
      ...
      <div class="fin-form-group">
        <label class="fin-form-label">Line Label Override <small style="color:#888;">(optional — prints on the invoice line instead of the Fee Item name)</small></label>
        <input type="text" id="rcv-fs-desc-override" class="fin-form-input" maxlength="200" placeholder="e.g. Tuition Fees – Acorns Half Day" value="${_finEsc(s?.description_override||'')}">
      </div>
...
async function submitFeeScheduleForm(editId) {
  ...
  const payload = {
    ...
    description_override: (document.getElementById('rcv-fs-desc-override')?.value || '').trim() || null,
  };
...
function _rcvInvBulkFeeItemHint() {
  ...
  const rows = items.length
    ? items.map(f => `<tr><td>${_finEsc(f.id)}</td><td>${_finEsc(f.code || '')}</td><td>${_finEsc(f.name || '')}</td></tr>`).join('')
    : `<tr><td colspan="3" class="fin-empty">${_finEsc(lookupPlaceholder('fee-items', 'No active fee items.'))}</td></tr>`;
  return `
...
async function loadFeeInvoicesView(container) {
  ...
  await _rcvLoadLookups({ terms:true, students:true, items:true });
  container.innerHTML = `
    <div class="fin-page">
      <div class="fin-header-row">
