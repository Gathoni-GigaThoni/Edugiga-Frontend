        ...
        <div class="flyout-nav sidebar" id="flyout-nav">
          ...
          <div class="flyout-module-body" id="flyout-body-inventory-management" data-label="Inventory" hidden>
            <ul id="inventory-management-dropdown" class="dropdown-menu">
              ...
              <li id="sidebar-inv-issues" onclick="loadView('inventory-issues')">Issues</li>
...
// any addition there must land here too, otherwise the deep-link will land
// on the list view without opening the target document.
const _DOC_ROUTE_TO_PRESELECT = {
  'journal-entries':             '_jeOpenId',
  'payables-payment-vouchers':   '_pvPvOpenId',
  'payables-supplier-invoices':  '_pvSiOpenId',
  // Wave 2 — Fee Invoice detail view reads _rcvCurrentInvoiceId directly
  // (not a preselectId; the view case builds the full detail page from
  // that single id). Credit Notes uses the split-view preselectId pattern.
  'fin-invoice-detail':          '_rcvCurrentInvoiceId',
  'fin-credit-notes':            '_cnOpenId',
  // Wave 3 — Receive Payments IS the Fund Receipt detail view. Reuses
  // the split-view preselectId pattern (var read + cleared inside
  // loadReceivePaymentsView).
  'fin-receive-payments':        '_rpOpenId',
  // Wave 5 — Payroll + Consultant runs. Both use module-level state
  // (_prSelectedRunId / _crSelectedRunId) rather than the split-view
  // preselect, so their loaders bridge window._prOpenId / _crOpenId
...
async function loadView(view) {
  ...
  switch(view) {
    ...
    case 'transport-bus-schedules':
      ...
      await loadTrnBusSchedulesView(main); break;
    ...
    case 'inventory-issues':
      setActiveSidebarItem('sidebar-inv-issues');
      await loadInventoryIssuesView(main); break;
    case 'inventory-transfers':
      setActiveSidebarItem('sidebar-inv-transfers');
      await loadInventoryTransfersView(main); break;
    case 'inventory-adjustments':
      setActiveSidebarItem('sidebar-inv-adjustments');
      await loadInventoryAdjustmentsView(main); break;
