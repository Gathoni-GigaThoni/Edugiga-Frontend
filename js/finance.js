...
async function _finBuildLedger(studentId) {
  ...
  return _toArray(data?.statement || []).map(e => ({
    ...
    // Clickable ref from the BE. Populated for invoice + credit_note;
    // null on receipts until Wave 3 lands the FUND_RECEIPT detail view.
    doc_ref:     e.doc_ref || null,
  }));
}
...
function _finLedgerTable(rows) {
  ...
  if (rows.length === 0) {
    ...
    rows.forEach(r => {
      ...
      // Reference cell: clickable when BE ships a doc_ref (invoice / CN);
      // plain text on receipts (Wave 3 will add the receipt link). Guard
      // with typeof so an unusual load order can't turn the whole
      // ledger into a ReferenceError.
      const refCell = (r.doc_ref && typeof _repDocLink === 'function')
        ? (_repDocLink(r.doc_ref) || _finEsc(r.reference || '—'))
        : _finEsc(r.reference || '—');
      bodyRows += `<tr>
...
async function loadReceivePaymentsView(container) {
  await _invLoadLookups();
  // Deep-link handoff: the hash router (dashboard.js:_maybeOpenDocFromHash)
  // and clickable doc_refs in Wave 3 reports set window._rpOpenId to pre-open
  // a specific receipt. One-shot: read + clear so a later plain
  // loadView('fin-receive-payments') doesn't stick to the old id.
  const preselectId = window._rpOpenId ?? null;
  window._rpOpenId = null;
  await renderSplitView({
    container,
