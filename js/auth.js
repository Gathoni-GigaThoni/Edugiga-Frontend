...
// Session-token bootstrap: config.js has already restored `token` and
// decoded `currentUser` from sessionStorage. If both are present the user
// has an unexpired session (config.js clears them on decode failure), so
// jump straight into the dashboard instead of re-prompting for
// credentials on every reload / new tab. showDashboard() then invokes
// _maybeOpenDocFromHash() so deep-link URLs open their target document.
// If a later API call 401s (server-side expiry / revocation) the existing
// 401 handler falls back to renderLoginPage().
