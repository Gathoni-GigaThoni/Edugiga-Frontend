...
async function _faRenderRegisterSplitView(container) {
  ...
  filterRow.innerHTML = `
    ...
    ${_faTab === 'live' ? `${canAdd('asset_management.depreciation') ? `<button class="fin-btn-outline" onclick="_faOpenDepreciationRunModal()">Run monthly depreciation</button>` : ''}
...
function _faDetailActions(item) {
  ...
  if (item.status === 'confirmed' && !item.is_disposed) {
    return `${_faGlActionsHtml(item)}${_faMovementActionsHtml(item)}
      ${canAdd('asset_management.disposal') ? `<button class="fin-btn-outline" onclick="_faOpenDisposeModal(${item.id})">Dispose</button>` : ''}
