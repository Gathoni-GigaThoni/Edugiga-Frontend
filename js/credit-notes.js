...
async function loadFinCreditNotesView(container) {
  ...
  // Deep-link handoff: hash router (js/dashboard.js:_maybeOpenDocFromHash)
  // and other modules set window._cnOpenId to pre-open a specific CN in
  // the split-view. One-shot: read + clear so a later plain
  // loadView('fin-credit-notes') doesn't stick to the old id.
  const preselectId = window._cnOpenId ?? null;
  window._cnOpenId = null;
